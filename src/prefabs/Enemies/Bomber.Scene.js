...
export default class Bomber extends Phaser.GameObjects.Sprite {
  ...
  createAnimations() {
    ...
    if (
      this.scene.textures.exists("bomber_explode") &&
      !this.scene.anims.exists("Bomber Explode")
    ) {
      this.scene.anims.create({
        key: "Bomber Explode",
        frames: this.scene.anims.generateFrameNumbers("bomber_explode", {
          start: 0,
          end: 5,
        }),
        frameRate: 15,
  ...
  updateAnimation() {
    if (this.isMoving) {
      if (
        ...
        this.anims.currentAnim.key !== "Bomber Run"
      ) {
      ...
      if (
        !this.anims.isPlaying ||
        this.anims.currentAnim.key !== "Bomber Idle"
      ) {
    ...
    try {
      if (this.scene.spawnExperienceOrb) {
        ...
        for (let i = 0; i < orbCount; i++) {
          ...
          this.scene.spawnExperienceOrb(this.x + xOffset, this.y + yOffset, 3);
        }
      }
    } catch (error) {
