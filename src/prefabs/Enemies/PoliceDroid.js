...
export default class PoliceDroid extends Phaser.GameObjects.Sprite {
    ...
    handleZombieCollision(zombie1, zombie2, policeDroid) {
        const distance = Phaser.Math.Distance.Between(
            zombie1.x, zombie1.y, zombie2.x, zombie2.y, policeDroid.x, policeDroid.y
        );
    ...
    createAnimations() {
        if (!this.scene.anims.exists('PoliceDroid')) {
            this.scene.anims.create({
                key: 'PoliceDroid',
                frames: this.scene.anims.generateFrameNumbers('Police run', { start: 0, end: 10}),
                frameRate: 8,
                repeat: -1
            });
        }
    ...
    updateAnimation() {
        if (this.isMoving) {
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'PoliceDroid') {
            ...
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'PoliceDroidIdle') {
