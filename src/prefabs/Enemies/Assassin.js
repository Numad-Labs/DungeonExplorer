...
export default class Assassin extends Phaser.GameObjects.Sprite {

    constructor(scene, x, y, texture, frame) {
        super(scene, x ?? 32, y ?? 32, texture || "Dagger Bandit-Run", frame ?? 0);
    ...
    createAnimations() {
        if (!this.scene.anims.exists('Assasin Run')) {
            this.scene.anims.create({
                key: 'Assasin Run',
                frames: this.scene.anims.generateFrameNumbers('Dagger Bandit-Run', { start: 0, end: 7}),
                frameRate: 8,
                repeat: -1
            });
        }
        if (!this.scene.anims.exists('assassin')) {
            this.scene.anims.create({
                key: 'assassin',
                frames: [{ key: 'Dagger Bandit-Run', frame: 0 }],
                frameRate: 1,
                repeat: 0
            });
        }
    ...
    updateAnimation() {
        if (this.isMoving) {
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'Assasin Run') {
                this.play('Assasin Run');
            }
            ...
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'assassin') {
                this.play('assassin');
            }
        }
    }
