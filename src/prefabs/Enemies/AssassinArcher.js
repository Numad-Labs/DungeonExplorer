...
export default class AssassinArcher extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene, x, y, texture, frame) {
        super(scene, x ?? 32, y ?? 32, texture || "Archer Bandit-Run_85x32", frame ?? 0);
    ...
    createAnimations() {
        if (!this.scene.anims.exists('AssasinArcher Run')) {
            this.scene.anims.create({
                ...
                frames: this.scene.anims.generateFrameNumbers('Archer Bandit-Run_85x32', { start: 0, end: 7}),
                frameRate: 8,
                repeat: -1
            });
        }
    ...
    createHealthBar() {
        ...
        this.healthBarFg = this.scene.add.rectangle(
            this.x + 55, this.y - 20, 30, 4, 0x00ff00
        );
    ...
    updateAnimation() {
        if (this.isMoving) {
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'AssasinArcher Run') {
            ...
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'AssasinArcher') {
