...
export default class Zombie2 extends Phaser.GameObjects.Sprite {
  constructor(scene, x, y, texture, frame) {
    super(scene, x ?? 32, y ?? 32, texture || "Zombie2RunAni", frame ?? 0);
  ...
  createAnimations() {
    if (!this.scene.anims.exists("zombieRunAni")) {
      this.scene.anims.create({
        key: "zombieRunAni",
        frames: this.scene.anims.generateFrameNumbers("Zombie2RunAni", {
          start: 0,
          end: 6,
        }),
        frameRate: 8,
    ...
    if (!this.scene.anims.exists("zombieIdle")) {
      this.scene.anims.create({
        key: "zombieIdle",
        frames: [{ key: "Zombie2RunAni", frame: 0 }],
        frameRate: 1,
        repeat: 0,
      });
    }
  ...
  updateAnimation() {
    if (this.isAttacking) {
      ...
      if (
        !this.anims.isPlaying ||
        this.anims.currentAnim.key !== "Zombie2RunAni"
      ) {
