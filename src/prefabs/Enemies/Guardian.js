...
export default class Guardian extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene, x, y, texture, frame) {
        super(scene, x ?? 64, y ?? 64, texture || "guradian_walk224x45", frame ?? 0);
    ...
    createAnimations() {
        if (!this.scene.anims.exists('Guardian Run')) {
            this.scene.anims.create({
                key: 'Guardian Run',
                frames: this.scene.anims.generateFrameNumbers('guradian_walk224x45', { start: 0, end: 7}),
                frameRate: 8,
                repeat: -1
            });
        }

        if (this.scene.textures.exists('guardian_attack') && !this.scene.anims.exists('Guardian Attack')) {
            this.scene.anims.create({
                key: 'Guardian Attack',
                frames: this.scene.anims.generateFrameNumbers('guardian_attack', { start: 0, end: 5}),
                frameRate: 10,
                repeat: 0 
            });
        }

        if (this.scene.textures.exists('guardian_charge') && !this.scene.anims.exists('Guardian Charge')) {
            this.scene.anims.create({
                key: 'Guardian Charge',
                frames: this.scene.anims.generateFrameNumbers('guardian_charge', { start: 0, end: 3}),
                frameRate: 15,
                repeat: -1 
            });
        }
    ...
    createHealthBar() {
        ...
        this.healthBarBg = this.scene.add.rectangle(this.x, this.y - 30, 60, 8, 0x660000);
        this.healthBarBg.setOrigin(0.5, 0.5);
        this.healthBarBg.setDepth(1);

        this.healthBarFg = this.scene.add.rectangle(
            this.x - 30, this.y - 30, 60, 8, 0x00ff00
        );
        this.healthBarFg.setOrigin(0, 0.5);
        this.healthBarFg.setDepth(20);

        this.shieldIndicator = this.scene.add.rectangle(this.x, this.y - 40, 60, 4, 0x00aaff);
    ...
    update(time, delta) {
        ...
        try {
            ...
            if (!this.isShielded && time - this.lastShieldTime > this.shieldAbilityCooldown && this.health < this.maxHealth * 0.5) {
                this.activateShield(time);
            }

            if (this.isShielded && time - this.shieldStartTime >= this.shieldDuration) {
                this.deactivateShield();
            }

            if (!this.isCharging && time - this.lastChargeTime > this.chargeAbilityCooldown && distance > 100 && distance < 300) {
    ...
    updateAnimation() {
        if (this.isCharging) {
            if (this.scene.anims.exists('Guardian Charge')) {
                if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'Guardian Charge') {
                    this.play('Guardian Charge');
                }
            } else {
                if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'Guardian Run') {
                    this.play('Guardian Run');
                }
            }
        } else if (this.isAttacking) {
            if (this.scene.anims.exists('Guardian Attack')) {
                if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'Guardian Attack') {
            ...
            if (!this.anims.isPlaying || this.anims.currentAnim.key !== 'Guardian Idle') {
