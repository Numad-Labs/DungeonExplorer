...
export default class UIManager {
  ...
  createTimer() {
    ...
    this.timer.background = this.scene.add
      .rectangle(config.x, config.y, config.width, config.height, 0x000000, 0.7)
      .setStrokeStyle(2, 0xffffff)
      .setScrollFactor(0)
      .setDepth(900);

    this.timer.label = this.scene.add
      .text(config.x, config.y - 20, "TIME", {
        fontFamily: "Arial",
        fontSize: "14px",
        color: "#ffffff",
        stroke: "#000000",
    ...
    this.timer.text = this.scene.add
      .text(config.x, config.y, "00:00", {
        fontFamily: "Arial",
        fontSize: "24px",
        color: "#ffff00",
        stroke: "#000000",
  ...
  createScoreboard() {
    ...
    this.scoreboard.background = this.scene.add
      .rectangle(0, 0, config.width, config.height, 0x000000, 0.7)
      .setStrokeStyle(2, 0xffffff);

    this.scoreboard.title = this.scene.add
      .text(0, -35, "STATS", {
        fontFamily: "Arial",
        fontSize: "14px",
        color: "#ffffff",
    ...
    this.scoreboard.stats.wave = this.scene.add.text(
      -60,
      -15,
      "Wave: 0",
      statStyle
    );
    this.scoreboard.stats.kills = this.scene.add.text(
      -60,
      0,
      "Kills: 0",
      statStyle
    );
    this.scoreboard.stats.gold = this.scene.add.text(
      -60,
      15,
      "Gold: 0",
      statStyle
  ...
  createAttackCooldownUI() {
    this.attackCooldowns.container = this.scene.add.container(
      80,
    ...
    attacks.forEach((attack) => {
      ...
      this.attackCooldowns[attack.key].background.setStrokeStyle(
        2,
        attack.color,
        1
      );
  ...
  updateAttackCooldowns() {
    if (!this.scene.playerAttackSystem || !this.attackCooldowns.container)
      return;
    ...
    attacks.forEach((attack) => {
      ...
      const cooldownProgress = Math.min(
        timeSinceLastAttack / attack.cooldown,
        1
      );
      ...
      if (cooldownProgress < 1) {
        ...
        this.attackCooldowns[attack.key].cooldownBar.fillCircle(
          attack.x,
          -120,
          11
        );
        this.attackCooldowns[attack.key].cooldownBar.fillStyle(
          attack.color,
          0.8
        );
        this.attackCooldowns[attack.key].cooldownBar.beginPath();
        this.attackCooldowns[attack.key].cooldownBar.moveTo(attack.x, -120);
        this.attackCooldowns[attack.key].cooldownBar.arc(
          attack.x,
          -120,
          11,
          -Math.PI / 2,
