...
export default class MobManager {
    ...
    getNearestWalkablePosition(worldX, worldY, maxSearchRadius = 10) {
        ...
        const randomPos = this.getRandomWalkablePosition();
        return randomPos ? { worldX: randomPos.worldX, worldY: randomPos.worldY } : { worldX, worldY };
    }
    ...
    teleportToWalkablePosition(entity, maxSearchRadius = 10) {
        ...
        const nearestWalkable = this.getNearestWalkablePosition(entity.x, entity.y, maxSearchRadius);
            ...
            for (let dx = -radius; dx <= radius; dx++) {
                for (let dy = -radius; dy <= radius; dy++) {
                    ...
                    if (tile && tile.index !== 0) {
                        ...
                        if (this.player) {
                            const distanceToPlayer = Phaser.Math.Distance.Between(
                                worldX, worldY, this.player.x, this.player.y
                            );
